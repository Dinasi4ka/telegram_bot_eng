...
from config import GOOGLE_CREDENTIALS_JSON, SPREADSHEET_ID_USERS, SPREADSHEET_ID_SCHEDULE, SHEET_USERS, SHEET_SCHEDULE
...
def _get_client() -> gspread.Client:
    """Повертає авторизований gspread клієнт."""
...
def _open_users_sheet() -> gspread.Worksheet:
    """Відкриває аркуш з окремого файлу Users."""
    client = _get_client()
    return client.open_by_key(SPREADSHEET_ID_USERS).worksheet(SHEET_USERS)


def _open_schedule_sheet() -> gspread.Worksheet:
    """Відкриває аркуш з окремого файлу Schedule."""
    client = _get_client()
    return client.open_by_key(SPREADSHEET_ID_SCHEDULE).worksheet(SHEET_SCHEDULE)
...
def get_all_users() -> list[dict]:
    return _open_users_sheet().get_all_records()


# ─────────────────────────── SCHEDULE ───────────────────────────

def get_free_slots() -> list[dict]:
    ws = _open_schedule_sheet()
    records = ws.get_all_records()
    free = []
    for i, row in enumerate(records, start=2):
...
def unbook_slot_by_phone_and_slot(date: str, time: str, phone: str) -> bool:
    ...
    for i, row in enumerate(records, start=2):
        if (str(row.get("date", "")) == date and
                str(row.get("time", "")) == time and
                _normalize_phone(str(row.get("booked_by_phone", ""))) == _normalize_phone(phone)):
            ws.update_cell(i, 3, "")
            ws.update_cell(i, 4, "")
            return True
    return False
...
def _normalize_phone(phone: str) -> str:
    """Прибирає пробіли, дужки, дефіси для порівняння."""
    import re
